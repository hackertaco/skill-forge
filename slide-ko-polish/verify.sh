...
if command -v claude >/dev/null 2>&1; then
  ...
  PROMPT="당신은 한국어 발표자료 카피라이터입니다. 다음 슬라이드 텍스트에서 영어 직역체와 어색한 표현만 찾아내세요.

[중요 컨텍스트]
- 슬라이드 텍스트입니다. 명사구 enumeration(예: 'A, B, C.')은 정상이므로 절대 지적하지 마세요.
- 키워드 나열(불릿)도 정상입니다.
- 영어 약어(verifier, AC, replay, fat-harness, ledger, run, auto, OS surface 등)는 의도적 용어이므로 한국어로 바꾸지 마세요.

[찾을 항목 — 이것만]
1. 무거운 인용 부호 + 명사구 단편 ('X'는 Y, 같은 구조의 단편 제목)
2. 추상 은유 직역 (닫는다/태우다/죽이다/살아있다 같은 영어 close/ship/kill/live 직역)
3. 영어 어순 그대로 옮긴 어색한 한국어 구조
4. 제목과 본문 메시지 불일치
...
문제 없으면 'PASS — 자연스러움' 한 줄만 답하세요.
띄어쓰기·오타는 무시하고 표현 자체의 어색함만 보세요.
